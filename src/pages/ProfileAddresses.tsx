...
// Import the map component (Assuming this file exists from your merge)
...
export const ProfileAddressesPage = () => {
  ...
  const handleLocationSelect = (lat: number, lng: number) => {
    console.log("Map selected:", lat, lng);
    setMapCoords({ lat, lng });
    // Optional: You could call a reverse geocoding API here to auto-fill city/country
  };
  ...
  const startEdit = (addr: AddressInterface) => {
    ...
    // If the address has saved coords, set them for the map (if your map supports showing a marker)
  ...
  const handleSaveAddress = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // If user didn't click the map, try to geocode the text address
    if (!finalLat || !finalLng) {
      const geocoded = await getLatLongFromAddress(
        form.address1,
        form.address2,
        form.city,
        form.pincode,
    ...
    if (editingId) {
      await updateSavedAddress(editingId, { ...form, lat: finalLat, lng: finalLng });
      setEditingId(null);
    } else {
      await saveAddressForUser(user, { ...form, lat: finalLat, lng: finalLng });
    }
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {addresses.map(addr => (
            <div key={addr.address_id} className="bg-white rounded-[2rem] shadow-sm border border-rose-100 p-6 hover:shadow-md transition-all">
              ...
              {addr.lat && addr.lng && (
                <p className="text-xs text-slate-400 mt-4 pl-11">📍 {addr.lat}, {addr.lng}</p>
              )}
      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-rose-100/50 border border-rose-100 p-8 mt-8">
        ...
        <form className="space-y-4" onSubmit={handleSaveAddress}>
          ...
          <div className="mb-6 h-64 rounded-2xl overflow-hidden border border-slate-200">
            {/* Passing the handler to the component your teammate created */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <input
              ...
              className="w-full p-4 bg-slate-50 border border-transparent rounded-xl focus:bg-white focus:border-rose-500 focus:ring-2 focus:ring-rose-200 outline-none transition-all font-medium col-span-2 md:col-span-1"
            />
          </div>
